...
function via_V = viability_V(C_E)
    ...
    via_V = v.E0 + (v.Einf - v.E0)/(1 + (v.EC50/C_E)^v.hill);
end
...
function d_V = death_V(C_E)
    d_V = 100 - viability_V(C_E); % death fraction of vulnerable (G1) cells
end
function d_R = death_R(C_E)
    d_R = 100 - viability_R(C_E); % death fraction of resistant (S/G2) cells
end
